...
async function getsongs(folder) {
  let a = await fetch(`http://127.0.0.1:2498/${folder}/`);
...
async function displayalbums() {
  let a = await fetch(`http://127.0.0.1:2498/songs`);
  let response = await a.text();
  // console.log(response)
  let div = document.createElement("div");
  div.innerHTML = response;
  ...
  for (let i = 0; i < p.length; i++) {
    if (p[i].href.includes("/songs/")) {
      ...
      let a = await fetch(`http://127.0.0.1:2498/songs/${folder}/info.json`);
        // console.log(a);
      let response = await a.json();
        
      let image = await fetch(`http://127.0.0.1:2498/songs/${folder}/image.json`);
      console.log(image);
